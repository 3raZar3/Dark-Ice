    ...
    switch (spellInfo->DmgClass)
    {
        ...
        case SPELL_DAMAGE_CLASS_MELEE:
        {
            ...
            if (crit)
            {
                ...
                // Resilience - reduce crit damage, is not ignored in any way
...
void Unit::CalculateAbsorbAndResist(Unit *pCaster, SpellSchoolMask schoolMask, DamageEffectType damagetype, const uint32 damage, uint32 *absorb, uint32 *resist, bool canReflect)
{
    ...
    if ((schoolMask & SPELL_SCHOOL_MASK_NORMAL)==0)
    {
        ...
        if (pCaster->GetTypeId() == TYPEID_UNIT && ((Creature*)pCaster)->isPet() && pCaster->GetOwner())
            victimResistance += (float)pCaster->GetOwner()->GetTotalAuraModifierByMiscMask(SPELL_AURA_MOD_TARGET_RESISTANCE, schoolMask);
